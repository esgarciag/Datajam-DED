...
def fig_contratacion_con_base() -> None:
    ...
    if not (ruta_tipo.exists() and ruta_ref.exists()):
        print("  (falta calendario o referencia — corré python -m src.contratos)")
        return
    ...
    for origen, destino in COPIAS.items():
        ...
        for ext in ("png", "svg"):
            ...
            if o.exists():
                ...
                faltan.append(ext)
        if faltan:
            print(f"  [falta] {origen}.{'/'.join(faltan)} — corré python -m src.viz")
        else:
