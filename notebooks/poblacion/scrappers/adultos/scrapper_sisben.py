...
# .../notebooks/poblacion/scrappers/adultos/scrapper_sisben.py
CARPETA_CODIGOS = Path(__file__).resolve().parent

# Raiz del repositorio:
# adultos < scrappers < poblacion < notebooks < raiz
...
# Un CSV crudo por barrio, tal como lo entrega el visor del Sisben.
CARPETA_DESCARGAS = RAIZ / "data" / "raw" / "sisben" / "adultos"

# Tabla consolidada de adultos por barrio, que consumen los demas analisis.
CARPETA_SALIDA = RAIZ / "data" / "processed" / "poblacion_adulta"
...
ARCHIVO_SALIDA_JSON = (
    CARPETA_SALIDA / "personas_Adultas_por_barrio.json"
)

ARCHIVO_SALIDA_CSV = (
    CARPETA_SALIDA / "personas_Adultas_por_barrio_resumen.csv"
)
            ...
            filas.append(
                {
                    "localidad": localidad,
                    "barrio": barrio,
                    "total_personas_adultas_raw": valor,
                }
            )

    tabla = pd.DataFrame(
        filas, columns=["localidad", "barrio", "total_personas_adultas_raw"]
    )

    valores_limpios = (
        tabla["total_personas_adultas_raw"]
        .astype(str)
        .str.replace(".", "", regex=False)
        .str.replace(",", "", regex=False)
        .str.strip()
    )
    tabla["total_personas_adultas"] = pd.to_numeric(valores_limpios, errors="coerce")
...
with sync_playwright() as p:
    ...
    # -----------------------------------------------------------
    # 1) Seleccionar "Adultos " en el filtro de edad
    # -----------------------------------------------------------
    dropdown_edad = abrir_selector(page, SELECTOR_EDAD)
    ok = seleccionar_opcion(page, dropdown_edad, "Adultos")
    print("¿Se seleccionó 'Adultos'?:", ok)
    ...
    # -----------------------------------------------------------
    # 4) Tabla final: localidad | barrio | total_personas_adultas
