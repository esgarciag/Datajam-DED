...
Estilo común y figuras del proyecto.
    ...
    python -m src.viz            # regenera todas las figuras disponibles

Las figuras se escriben en outputs/figures/ en PNG (300 dpi) y SVG.
...
def guardar(fig: plt.Figure, nombre: str) -> None:
    ...
    plt.close(fig)
    print(f"  figura -> outputs/figures/{nombre}.png|svg")
...
def fig_curva_ejecucion_idrd() -> None:
    ...
    if not ruta.exists():
        print("  (falta ejecucion_idrd_mensual.csv — corré python -m src.ejecucion)")
        return
    m = pd.read_csv(ruta)
...
def fig_calendario_contratacion() -> None:
    ...
    if not ruta.exists():
        print("  (falta contratos_calendario_tipo.csv — corré python -m src.contratos)")
        return
    d = pd.read_csv(ruta)
...
def fig_ejecucion_fdl() -> None:
    ...
    if not ruta.exists():
        print("  (falta ejecucion_fdl_anual.csv — corré python -m src.ejecucion)")
        return
    d = pd.read_csv(ruta)
...
def fig_cuadrantes() -> None:
    ...
    if not ruta.exists():
        print("  (falta cruce_final.csv — corré python -m src.cruce)")
        return
    d = pd.read_csv(ruta)
...
def fig_plata_vs_servicio() -> None:
    ...
    if "sesiones_pc_100k" not in d.columns:
        print("  (falta la capa de oferta — corré python -m src.oferta)")
        return
    d = d[~d["localidad"].isin(OUTLIERS_TAMANO)].copy()
...
def fig_curvas_concentracion() -> None:
    ...
    if "sesiones" not in d.columns:
        print("  (falta la capa de oferta — corré python -m src.oferta y src.cruce)")
        return
...
def fig_matriz_equidad() -> None:
    ...
    if not ruta.exists():
        print("  (falta matriz_equidad.csv — corré python -m src.equidad)")
        return
    import numpy as np
...
def fig_avance_fisico() -> None:
    ...
    if not ruta.exists():
        print("  (falta desempeno_actividades_af.csv — corré python -m src.desempeno)")
        return
...
def fig_promesa_vs_realidad() -> None:
    ...
    if not ruta.exists():
        print("  (falta brecha_catalogo_60mas.csv — corré python -m src.scrape_60mas)")
        return
    b = pd.read_csv(ruta)
...
def main() -> int:
    ...
    if not ruta.exists():
        print("Falta data/processed/riesgo_localidad.csv — corré: python -m src.riesgo")
        return 1
